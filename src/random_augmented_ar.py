...
def forward_process_t(batch, t, generator, mask_id):
    ...
    maskable_indices = batch["maskable"].to(device)
    
    random_token_ids = torch.randint(
        low=0,
        high=127999,  # generates values in [0, 127999]
    ...
    noisy_input_ids = torch.where(maskable_indices, random_token_ids, input_ids)

    batch["noisy_input_ids"] = noisy_input_ids
    batch["masked_indices"] = maskable_indices
...
def main():
    ...
    checkpoint_filename = f"random_AR_seed_{seed}_lr_{learning_rate}_time_{current_time}_numstories{no_of_stories}_{dataset_name}.json" #get_checkpoint_filename(checkpoint_config)
    ...
    run_name = f"random_AR_fullparam_seed_{seed}_{dataset_name}_lr_{learning_rate}_epoch_{num_train_epochs}_paraphrases_{paraphrases}_t_mode_{t_mode}_t_{t}"
    ...
    if save_checkpoint:
        model_checkpoint_dir = "./checkpoints/random_ar/"
        os.makedirs(model_checkpoint_dir, exist_ok=True)
        model_checkpoint_path = os.path.join(model_checkpoint_dir, f"random_AR_fullparam_seed_{seed}_{dataset_name}_lr_{learning_rate}_epoch_{num_train_epochs}_time_{current_time}")
