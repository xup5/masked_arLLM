...
def main():
    ...
    parser.add_argument('--save_path', type=str, default="./results/ar/", help='Path to save results')
    ...
    run_name = f"AR_fullparam_seed_{seed}_{dataset_name}_lr_{learning_rate}_epoch_{num_train_epochs}_paraphrases_{paraphrases}_batchsize_{batch_size}_gradacc_{gradient_accumulation_steps}"
    ...
    data_loader = get_training_dataloader(dataset_name, tokenizer, Dataset_pretraining_format, paraphrases=paraphrases, batch_size=batch_size, device=device, model_type="llama")
    ...
    while actual_epoch <= num_train_epochs:
        ...
        # try:
        #     generator.manual_seed(seed + rank + actual_epoch * 1000)
        # except Exception:
        #     pass
    ...
    if save_checkpoint:
        model_checkpoint_dir = "./checkpoints/ar/"
        os.makedirs(model_checkpoint_dir, exist_ok=True)
        model_checkpoint_path = os.path.join(model_checkpoint_dir, f"AR_fullparam_seed_{seed}_{dataset_name}_lr_{learning_rate}_epoch_{num_train_epochs}_time_{current_time}")
